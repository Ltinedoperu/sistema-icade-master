...
const SimpleBarChart = ({ data, isDark }) => {
    ...
    const textColor = isDark ? 'text-slate-500' : 'text-slate-600 font-bold';
    const borderColor = isDark ? 'border-slate-700' : 'border-slate-300';
    const tooltipBg = isDark ? 'bg-black text-white' : 'bg-white text-black border shadow-sm';
...
const Dashboard = () => {
  ...
  // --- DICCIONARIO DE COLORES (LA SOLUCIÓN SEGURA) ---
  const colors = {
      bg: darkMode ? 'bg-[#0B1120]' : 'bg-slate-100', // Fondo gris suave en light
      text: darkMode ? 'text-slate-200' : 'text-slate-800',
      textDim: darkMode ? 'text-slate-400' : 'text-slate-500',
      card: darkMode ? 'bg-[#151e32] border-slate-800' : 'bg-white border-slate-300 shadow-sm', // Tarjetas blancas en light
      ...
      tableHead: darkMode ? 'bg-[#0f1623] text-amber-500' : 'bg-slate-200 text-slate-800 border-b border-slate-300',
  ...
  const abrirFicha = async (venta) => {
    ...
    setHistorialPagos([]); // Limpiamos para evitar error visual
  ...
  return (
    <div className={`min-h-screen font-sans pb-20 relative transition-colors duration-300 ${colors.bg} ${colors.text}`}>
      
      {/* NAVBAR */}
      <nav className={`${colors.nav} px-6 py-4 flex flex-col md:flex-row justify-between items-center gap-4 sticky top-0 z-40 border-b transition-colors duration-300`}>
        <div className="flex items-center gap-3">
            ...
            <div>
                ...
                <p className={`text-[10px] uppercase font-bold ${colors.textDim}`}>Rol: {currentUserRole}</p>
            </div>
        </div>
        <div className="flex gap-4 overflow-x-auto items-center">
          
          {/* BOTÓN TEMA */}
          <button onClick={toggleTheme} className={`p-2 rounded-lg border transition-all ${colors.btnGhost}`}>
          ...
          <div className={`flex rounded-lg p-1 ${darkMode ? 'bg-slate-800' : 'bg-slate-200'}`}>
             {['ventas', 'reportes', 'cursos', 'equipo'].map((v) => (
                 <button key={v} onClick={() => setView(v)} className={`px-4 py-1.5 rounded-md text-sm font-bold whitespace-nowrap capitalize transition-all ${view === v ? 'bg-amber-500 text-[#0B1120]' : `${colors.textDim} hover:text-slate-600`}`}>{v}</button>
             ))}
          </div>
          <button onClick={handleLogout} className={`p-2 rounded-lg text-rose-500 border transition-all ${colors.btnGhost}`}><LogOut size={20} /></button>
        </div>
      </nav>
      ...
      <main className="max-w-[98%] mx-auto p-4 md:p-6 transition-colors duration-300">
        ...
        {view === 'ventas' && (
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
                <div className={`${colors.card} p-5 rounded-2xl border flex items-center gap-4`}><div className="p-3 bg-emerald-500/10 rounded-xl text-emerald-500"><TrendingUp size={24} /></div><div><p className={`text-sm ${colors.textDim}`}>Hoy</p><h3 className="text-2xl font-bold">{stats.hoy}</h3></div></div>
                <div className={`${colors.card} p-5 rounded-2xl border flex items-center gap-4`}><div className="p-3 bg-blue-500/10 rounded-xl text-blue-500"><BarChart3 size={24} /></div><div><p className={`text-sm ${colors.textDim}`}>Mes</p><h3 className="text-2xl font-bold">{stats.mes}</h3></div></div>
                <div className={`${colors.card} p-5 rounded-2xl border flex items-center gap-4`}><div className="p-3 bg-amber-500/10 rounded-xl text-amber-500"><Users size={24} /></div><div><p className={`text-sm ${colors.textDim}`}>Total</p><h3 className="text-2xl font-bold">{stats.total}</h3></div></div>
            </div>

            <div className={`${colors.card} p-4 rounded-2xl border mb-6 animate-fadeIn`}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-5 gap-3">
                    <div className="relative"><Search className="absolute left-3 top-2.5 text-slate-500" size={16}/><input type="text" placeholder="Buscar..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className={`w-full border rounded-xl py-2 px-4 pl-9 text-xs outline-none ${colors.input}`}/></div>
                    <select value={filterPromoter} onChange={(e) => setFilterPromoter(e.target.value)} className={`border rounded-xl py-2 px-3 text-xs outline-none ${colors.input}`}><option value="">Todos los Promotores</option>{usuarios.filter(u => u.rol === 'promotor').map(u => (<option key={u.id} value={u.email}>{u.nombre} {u.apellidos}</option>))}</select>
                    <input type="text" placeholder="Ciudad" value={filterCity} onChange={(e) => setFilterCity(e.target.value)} className={`border rounded-xl py-2 px-3 text-xs outline-none ${colors.input}`}/>
                    <input type="text" placeholder="UGEL" value={filterUgel} onChange={(e) => setFilterUgel(e.target.value)} className={`border rounded-xl py-2 px-3 text-xs outline-none ${colors.input}`}/>
                    <div className="flex gap-1"><input type="date" value={filterDateStart} onChange={(e) => setFilterDateStart(e.target.value)} className={`w-1/2 border rounded-xl p-1 text-[10px] text-center ${colors.input}`}/><input type="date" value={filterDateEnd} onChange={(e) => setFilterDateEnd(e.target.value)} className={`w-1/2 border rounded-xl p-1 text-[10px] text-center ${colors.input}`}/></div>
                </div>
            </div>

            <div className={`${colors.card} rounded-2xl border overflow-hidden shadow-xl`}>
                <div className="overflow-x-auto">
                    <table className="w-full text-sm text-left border-collapse">
                        <thead className={`${colors.tableHead} font-bold text-xs uppercase`}>
                            <tr>
                        ...
                        <tbody className={`divide-y ${colors.divider}`}>
                            {loading ? <tr><td colSpan="7" className="p-8 text-center text-slate-500">Cargando...</td></tr> : filteredVentas.map((venta) => (
                                <tr key={venta.id} className={`${colors.tableRow} transition-colors`}>
                                    ...
                                    <td className="px-6 py-4 border-b border-inherit"><div className="flex flex-col"><span className={`font-mono text-xs ${colors.textDim}`}>{new Date(venta.created_at).toLocaleDateString()}</span><span className="text-[10px] text-emerald-500 font-bold uppercase mt-1">{obtenerNombreAsesor(venta.promotor_email, venta.promotor_nombre)}</span></div></td>
                                    <td className="px-6 py-4 border-b border-inherit">
                                        <div className="font-bold flex items-center gap-2">{venta.nombre}</div>
                                        <div className={`text-xs ${colors.textDim}`}>{venta.dni}</div>
                                        <div className={`text-[10px] ${colors.textDim} mt-1 opacity-70`}>Ficha: {venta.numero_ficha_fisica || 'S/N'}</div>
                                    </td>
                                    <td className="px-6 py-4 border-b border-inherit"><div className="text-sm">{venta.condicion_laboral}</div><div className={`text-xs ${colors.textDim}`}>{venta.institucion}</div><div className={`text-[10px] ${colors.textDim}`}>{venta.ciudad}</div></td>
                                    ...
                                    <td className="px-6 py-4 border-b border-inherit text-xs max-w-xs">
                                        ...
                                        <span className={`truncate block ${colors.textDim}`}>{venta.programa}</span>
                                    </td>
                                    <td className="px-6 py-4 border-b border-inherit text-center">
                                        <div className="flex justify-center gap-2">
                                            <button onClick={() => abrirFicha(venta)} className={`p-2 rounded border transition-colors ${colors.btnGhost} hover:text-blue-500 hover:border-blue-500`} title="Ver Ficha"><Eye size={16}/></button>
                                            {currentUserRole !== 'supervisor' && (
                                                <>
                                                    <button onClick={() => abrirEditar(venta)} className={`p-2 rounded border transition-colors ${colors.btnGhost} hover:text-amber-500 hover:border-amber-500`} title="Editar"><Edit size={16}/></button>
                                                    <button onClick={() => abrirPagar(venta)} className={`p-2 rounded border transition-colors ${colors.btnGhost} hover:text-emerald-500 hover:border-emerald-500`} title="Pagar"><DollarSign size={16}/></button>
                                                </>
                                            )}
                                        </div>
                                    </td>
        ...
        {view === 'reportes' && (
            <div className="space-y-8 animate-fadeIn">
                <div className="flex justify-between items-center">
                    ...
                    <div className="flex gap-2"><input type="date" className={`border rounded p-2 text-xs ${colors.input}`}/><input type="date" className={`border rounded p-2 text-xs ${colors.input}`}/></div>
                </div>
                <div className={`${colors.card} p-6 rounded-2xl border`}><h3 className="font-bold mb-4">Ventas Diarias (Mes Actual)</h3><SimpleBarChart data={reportData.daily} isDark={darkMode} /></div>
            </div>
        )}

        {view === 'equipo' && (
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div className={`${colors.card} p-6 rounded-2xl border h-fit`}>
                    ...
                    <p className={`text-xs text-center ${colors.textDim}`}>La clave de acceso será el número de DNI.</p>
                </div>
                <div className="lg:col-span-2 space-y-4">{usuarios.map(u => (<div key={u.id} className={`p-5 rounded-2xl border flex justify-between items-center ${!u.activo ? 'opacity-60' : ''} ${colors.card}`}><div className="flex items-center gap-3"><div className={`w-12 h-12 rounded-full flex items-center justify-center font-bold ${darkMode ? 'bg-slate-700 text-white' : 'bg-slate-200 text-slate-700'}`}>{u.nombre?.charAt(0).toUpperCase()}</div><div><h4 className="font-bold">{u.nombre} {u.apellidos}</h4><p className={`text-sm ${colors.textDim}`}>{u.rol} • {u.dni}</p></div></div><div className="flex gap-2">
        ...
        {view === 'cursos' && (
             <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div className={`${colors.card} p-6 rounded-2xl border h-fit`}>
                    ...
                    {currentUserRole !== 'supervisor' && (
                        <form onSubmit={handleAddCurso} className="space-y-4"><select className={`w-full border rounded-xl p-3 ${colors.input}`} value={nuevoCurso.nivel} onChange={e=>setNuevoCurso({...nuevoCurso, nivel:e.target.value})}><option value="Inicial">Inicial</option><option value="Primaria">Primaria</option><option value="Secundaria">Secundaria</option></select><input className={`w-full border rounded-xl p-3 ${colors.input}`} placeholder="Nombre del curso" value={nuevoCurso.nombre} onChange={e=>setNuevoCurso({...nuevoCurso, nombre:e.target.value})}/><button className="w-full bg-amber-600 p-3 rounded-xl font-bold text-black">Guardar</button></form>
                    )}
                </div>
                <div className="lg:col-span-2 space-y-2">{cursos.map(c=><div key={c.id} className={`${colors.card} p-4 rounded-xl border flex justify-between`}><div><span className="text-xs text-amber-500 block uppercase font-bold">{c.nivel}</span><span className="">{c.nombre}</span></div>
      ...
      {modalEditOpen && editForm && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <div className={`w-full max-w-3xl rounded-3xl border shadow-2xl p-6 relative max-h-[90vh] overflow-y-auto z-10 ${colors.modalBg}`}>
                <button onClick={() => setModalEditOpen(false)} className={`absolute top-4 right-4 p-2 rounded-full transition-colors z-20 ${colors.btnGhost}`}><X size={20}/></button>
                ...
                <form onSubmit={guardarEdicion} className="space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                        <div><label className={`text-xs ml-1 ${colors.textDim}`}>Nombre</label><input className={`w-full border rounded-xl p-3 ${colors.input}`} value={editForm.nombre} onChange={e=>setEditForm({...editForm, nombre:e.target.value})}/></div>
                        <div><label className={`text-xs ml-1 ${colors.textDim}`}>DNI</label><input className={`w-full border rounded-xl p-3 ${colors.input}`} value={editForm.dni} onChange={e=>setEditForm({...editForm, dni:e.target.value})}/></div>
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                        <div><label className={`text-xs ml-1 ${colors.textDim}`}>Tipo</label><select className={`w-full border rounded-xl p-3 ${colors.input}`} value={editForm.tipo_registro} onChange={e=>setEditForm({...editForm, tipo_registro:e.target.value})}><option>Diplomado</option><option>Especialización</option><option>Curso de Capacitación</option><option>Curso de Actualización</option><option>Nombramiento Docente</option><option>Ascenso Docente</option></select></div>
                        <div><label className={`text-xs ml-1 ${colors.textDim}`}>Modalidad</label><select className={`w-full border rounded-xl p-3 ${colors.input}`} value={editForm.modalidad_estudio} onChange={e=>setEditForm({...editForm, modalidad_estudio:e.target.value})}><option>Programa Completo</option><option>Acelerada</option></select></div>
                    </div>
                    <div><label className={`text-xs ml-1 ${colors.textDim}`}>Cursos Inscritos (Programa)</label><textarea className={`w-full border rounded-xl p-3 h-24 text-sm font-mono ${colors.input}`} value={editForm.programa || ''} onChange={e=>setEditForm({...editForm, programa:e.target.value})}/></div>
                    <div className="grid grid-cols-2 gap-4">
                        <div><label className={`text-xs ml-1 ${colors.textDim}`}>Celular</label><input className={`w-full border rounded-xl p-3 ${colors.input}`} value={editForm.celular} onChange={e=>setEditForm({...editForm, celular:e.target.value})}/></div>
                        <div><label className={`text-xs ml-1 ${colors.textDim}`}>WhatsApp</label><input className={`w-full border rounded-xl p-3 ${colors.input}`} value={editForm.whatsapp} onChange={e=>setEditForm({...editForm, whatsapp:e.target.value})}/></div>
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                        <div><label className={`text-xs ml-1 ${colors.textDim}`}>Centro Laboral</label><input className={`w-full border rounded-xl p-3 ${colors.input}`} value={editForm.institucion} onChange={e=>setEditForm({...editForm, institucion:e.target.value})}/></div>
                        <div><label className={`text-xs ml-1 ${colors.textDim}`}>UGEL</label><input className={`w-full border rounded-xl p-3 ${colors.input}`} value={editForm.ugel} onChange={e=>setEditForm({...editForm, ugel:e.target.value})}/></div>
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                        <div><label className={`text-xs ml-1 ${colors.textDim}`}>Ficha Física</label><input className={`w-full border rounded-xl p-3 ${colors.input}`} value={editForm.numero_ficha_fisica} onChange={e=>setEditForm({...editForm, numero_ficha_fisica:e.target.value})}/></div>
                        <div><label className={`text-xs ml-1 ${colors.textDim}`}>Ciudad</label><input className={`w-full border rounded-xl p-3 ${colors.input}`} value={editForm.ciudad} onChange={e=>setEditForm({...editForm, ciudad:e.target.value})}/></div>
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                        <div><label className={`text-xs ml-1 ${colors.textDim}`}>Condición</label><select className={`w-full border rounded-xl p-3 ${colors.input}`} value={editForm.condicion_laboral} onChange={e=>setEditForm({...editForm, condicion_laboral:e.target.value})}><option>Nombrado</option><option>Contratado</option><option>Sin Vínculo</option></select></div>
                        <div><label className={`text-xs ml-1 ${colors.textDim}`}>Pago</label><select className={`w-full border rounded-xl p-3 ${colors.input}`} value={editForm.modalidad_pago} onChange={e=>setEditForm({...editForm, modalidad_pago:e.target.value})}><option>Pago a Cuenta</option><option>Descuento por Planilla</option></select></div>
                    </div>
                    <div><label className={`text-xs ml-1 ${colors.textDim}`}>Observaciones</label><textarea className={`w-full border rounded-xl p-3 h-24 ${colors.input}`} value={editForm.observaciones || ''} onChange={e=>setEditForm({...editForm, observaciones:e.target.value})}/></div>
      ...
      {modalFichaOpen && fichaData && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <div className={`w-full max-w-5xl rounded-3xl border shadow-2xl relative my-8 overflow-y-auto max-h-[95vh] z-10 flex flex-col ${colors.modalBg} ${isDark ? 'border-slate-700' : 'border-slate-200'}`}>
                <div className={`flex justify-between items-center p-6 border-b rounded-t-3xl sticky top-0 z-20 ${darkMode ? 'bg-[#0f1623] border-slate-800' : 'bg-white border-slate-200'}`}>
                    <div>
                        ...
                        <p className={`text-sm flex gap-2 ${colors.textDim}`}><span>{fichaData.dni}</span> • <span className="text-emerald-500 font-bold">{fichaData.modalidad_estudio}</span></p>
                    </div>
                    <button onClick={() => setModalFichaOpen(false)} className={`p-2 rounded-full transition-colors ${colors.btnGhost}`}><X size={24}/></button>
                </div>
                ...
                <div className="p-8 space-y-8 overflow-y-auto">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div className="space-y-4">
                            ...
                            <div className="grid grid-cols-2 gap-4 text-sm">
                                <div><span className={`block ${colors.textDim}`}>Celular</span> <span>{fichaData.celular}</span></div>
                                <div><span className={`block ${colors.textDim}`}>WhatsApp</span> <span>{fichaData.whatsapp || '-'}</span></div>
                                <div className="col-span-2"><span className={`block ${colors.textDim}`}>Correo</span> <span>{fichaData.correo || '-'}</span></div>
                                <div className="col-span-2"><span className={`block ${colors.textDim}`}>Dirección</span> <span>{fichaData.direccion || '-'}</span></div>
                                <div><span className={`block ${colors.textDim}`}>Ciudad</span> <span>{fichaData.ciudad || '-'}</span></div>
                                <div><span className={`block ${colors.textDim}`}>Ficha Física</span> <span className="font-bold">{fichaData.numero_ficha_fisica || 'N/A'}</span></div>
                            </div>
                        </div>
                        <div className="space-y-4">
                            ...
                            <div className="grid grid-cols-2 gap-4 text-sm">
                                <div><span className={`block ${colors.textDim}`}>Condición</span> <span>{fichaData.condicion_laboral}</span></div>
                                <div><span className={`block ${colors.textDim}`}>Nivel</span> <span>{fichaData.nivel || '-'}</span></div>
                                <div className="col-span-2"><span className={`block ${colors.textDim}`}>Centro Laboral</span> <span>{fichaData.institucion || 'Sin IE'}</span></div>
                                <div className="col-span-2"><span className={`block ${colors.textDim}`}>UGEL / DRE</span> <span>{fichaData.ugel || '-'}</span></div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                            <div>
                                <span className={`block text-xs ${colors.textDim}`}>Cursos Inscritos</span>
                            ...
                            <div className="bg-emerald-500/10 p-4 rounded-xl border border-emerald-500/20">
                                ...
                                <div className="grid grid-cols-2 gap-4 text-sm">
                                    <div><span className={`block ${colors.textDim}`}>Modalidad</span> <span>{fichaData.modalidad_pago}</span></div>
                                    <div><span className={`block ${colors.textDim}`}>Total Pactado</span> <span className="text-amber-500 font-bold">S/ {fichaData.total_pagar || '0.00'}</span></div>
                                    <div><span className={`block ${colors.textDim}`}>Cuotas</span> <span>{fichaData.num_cuotas || '1'}</span></div>
                                    <div><span className={`block ${colors.textDim}`}>Mensualidad</span> <span>S/ {fichaData.monto_mensual || '0.00'}</span></div>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className={`pt-4 border-t ${darkMode ? 'border-slate-800' : 'border-slate-200'}`}>
                        ...
                        <div className={`overflow-hidden rounded-xl border ${darkMode ? 'border-slate-700' : 'border-slate-200'}`}>
                            <table className="w-full text-sm text-left">
                                <thead className={colors.tableHead}><tr><th className="px-4 py-3">Fecha</th><th className="px-4 py-3">Concepto</th><th className="px-4 py-3">Medio</th><th className="px-4 py-3 text-right">Monto</th></tr></thead>
                                <tbody className={`divide-y ${darkMode ? 'divide-slate-800 bg-[#151e32]' : 'divide-slate-200 bg-white'}`}>
                                    {historialPagos.length === 0 ? <tr><td colSpan="4" className={`p-6 text-center italic ${colors.textDim}`}>No hay pagos registrados aún.</td></tr> : 
                                    historialPagos.map((pago) => (
                                        <tr key={pago.id} className={colors.tableRow}>
                                            <td className={`px-4 py-3 font-mono text-xs ${colors.textDim}`}>{pago.fecha_pago}</td>
      ...
      {modalPayOpen && payData && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <div className={`w-full max-w-2xl rounded-3xl border shadow-2xl p-6 relative z-10 ${colors.modalBg}`}>
                <button onClick={() => setModalPayOpen(false)} className={`absolute top-4 right-4 p-2 rounded-full transition-colors ${colors.btnGhost}`}><X size={20}/></button>
                <div className="mb-6">
                    <h2 className="text-2xl font-bold flex items-center gap-2"><DollarSign className="text-emerald-500"/> Registrar Pago</h2>
                    <p className={`text-sm mt-1 font-mono ${colors.textDim}`}>{payData.nombre} • DNI: {payData.dni}</p>
                </div>
                <form onSubmit={registrarPago} className={`space-y-4 p-6 rounded-2xl border ${darkMode ? 'bg-emerald-900/10 border-emerald-500/20' : 'bg-emerald-50 border-emerald-200'}`}>
                    <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-1"><label className={`text-[10px] uppercase font-bold ${colors.textDim}`}>Concepto (Mes)</label><input className={`w-full border rounded-lg p-3 text-sm ${colors.input}`} placeholder="Ej. Enero 2026" value={nuevoPago.concepto} onChange={e=>setNuevoPago({...nuevoPago, concepto:e.target.value})} required/></div>
                        <div className="space-y-1"><label className={`text-[10px] uppercase font-bold ${colors.textDim}`}>Monto (S/)</label><input type="number" step="0.01" className={`w-full border rounded-lg p-3 text-sm font-bold text-emerald-500 ${colors.input}`} placeholder="0.00" value={nuevoPago.monto} onChange={e=>setNuevoPago({...nuevoPago, monto:e.target.value})} required/></div>
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-1"><label className={`text-[10px] uppercase font-bold ${colors.textDim}`}>Medio de Pago</label>
                        <select className={`w-full border rounded-lg p-3 text-sm ${colors.input}`} value={nuevoPago.medio_pago} onChange={e=>setNuevoPago({...nuevoPago, medio_pago:e.target.value})}>
                        ...
                        <div className="space-y-1"><label className={`text-[10px] uppercase font-bold ${colors.textDim}`}>Fecha de Pago</label><input type="date" className={`w-full border rounded-lg p-3 text-sm ${colors.input}`} value={nuevoPago.fecha_pago} onChange={e=>setNuevoPago({...nuevoPago, fecha_pago:e.target.value})} required/></div>
                    </div>
      ...
      {modalUserOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <div className={`w-full max-w-lg rounded-3xl border shadow-2xl p-8 relative z-10 ${colors.modalBg}`}>
                <button onClick={() => setModalUserOpen(false)} className={`absolute top-4 right-4 p-2 rounded-full transition-colors ${colors.btnGhost}`}><X size={20}/></button>
                ...
                <form onSubmit={guardarUsuario} className="space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                        <div><label className={`text-xs ml-1 ${colors.textDim}`}>Nombres</label><input className={`w-full border p-3 rounded-xl ${colors.input}`} value={userForm.nombre} onChange={e=>setUserForm({...userForm, nombre:e.target.value})} required/></div>
                        <div><label className={`text-xs ml-1 ${colors.textDim}`}>Apellidos</label><input className={`w-full border p-3 rounded-xl ${colors.input}`} value={userForm.apellidos} onChange={e=>setUserForm({...userForm, apellidos:e.target.value})} required/></div>
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                        <div><label className={`text-xs ml-1 ${colors.textDim}`}>DNI (Será la clave)</label><input className={`w-full border p-3 rounded-xl ${colors.input}`} value={userForm.dni} onChange={e=>setUserForm({...userForm, dni:e.target.value})} required/></div>
                        <div><label className={`text-xs ml-1 ${colors.textDim}`}>Celular</label><input className={`w-full border p-3 rounded-xl ${colors.input}`} value={userForm.celular} onChange={e=>setUserForm({...userForm, celular:e.target.value})} required/></div>
                    </div>
                    <div><label className={`text-xs ml-1 ${colors.textDim}`}>Correo Electrónico</label><input type="email" className={`w-full border p-3 rounded-xl ${colors.input}`} value={userForm.email} onChange={e=>setUserForm({...userForm, email:e.target.value})} required/></div>
                    
                    {/* SELECTOR DE ROLES */}
                    <div>
                        <label className={`text-xs ml-1 ${colors.textDim}`}>Rol de Acceso</label>
                        <select className={`w-full border p-3 rounded-xl outline-none ${colors.input}`} value={userForm.rol} onChange={e=>setUserForm({...userForm, rol:e.target.value})}>
