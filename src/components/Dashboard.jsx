                ...
                return (
                    <div key={i} className="flex-1 flex flex-col items-center gap-2 group relative">
                        <div 
                            ...
                            style={{ height: `${porcentaje}%`, opacity: 0.8 }}
                        >
...
const Dashboard = () => {
  ...
  // Función Blindada Anti-Crash
  const obtenerNombreAsesor = (email) => {
    if (!email || typeof email !== 'string') return 'Sin Asignar';
    if (!usuarios || usuarios.length === 0) return email.split('@')[0];
  ...
  // Función corregida para evitar Pantalla Azul
  const abrirFicha = async (v) => {
    if (!v) return;
    setFichaData(v);
    setHistorialPagos([]); // Limpiamos datos viejos para evitar errores
  ...
  return (
    <div className={`min-h-screen font-sans pb-20 transition-colors duration-300 ${theme.bg} ${theme.textMain}`}>
      ...
      {/* MODALES CORREGIDOS (Blindados contra fallos) */}
                ...
                <div className="p-6 overflow-y-auto flex-1 space-y-6">
                    ...
                    <div className={`pt-4 border-t ${darkMode?'border-slate-700':'border-slate-300'}`}>
                        <h3 className={`text-lg font-bold mb-4 flex items-center gap-2 ${theme.textMain}`}><DollarSign className="text-emerald-500"/> Pagos</h3>
                         {(historialPagos || []).length===0 ? <p className="text-slate-500 italic">Sin pagos.</p> : (
                             <div className="overflow-hidden">
                                <table className={`w-full text-sm ${theme.tableDivide}`}>
                                    <thead className={theme.tableHeader}><tr><th className="p-2 text-left">Fecha</th><th className="p-2 text-left">Concepto</th><th className="p-2 text-right">Monto</th></tr></thead>
                                    <tbody className={`divide-y ${theme.tableDivide}`}>{(historialPagos || []).map(p=>(<tr key={p.id}><td className={`p-2 ${theme.textSec}`}>{p.fecha_pago}</td><td className={`p-2 font-bold ${theme.textMain}`}>{p.concepto}</td><td className="p-2 text-right font-black text-emerald-500">S/ {p.monto}</td></tr>))}</tbody>
                                    <tfoot><tr><td colSpan="2" className="p-2 text-right font-bold">TOTAL:</td><td className="p-2 text-right font-black text-emerald-500">S/ {(historialPagos || []).reduce((acc, curr) => acc + (parseFloat(curr.monto) || 0), 0).toFixed(2)}</td></tr></tfoot>
                                </table>
                             </div>
                         )}
                    </div>
