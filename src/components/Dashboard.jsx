...
import { LogOut, Search, FileText, User, Plus, Trash2, Layers, MapPin, Edit, X, BarChart3, TrendingUp, Users, Shield, Power, Eye, Download, Share2, DollarSign, Check, XCircle, PieChart, Filter, Sun, Moon, Calendar } from 'lucide-react';
            ...
            {data.map((d, i) => (
                <div key={i} className="flex-1 flex flex-col items-center gap-1 group">
                    <div className="w-full bg-emerald-500/20 rounded-t hover:bg-emerald-500/40 transition-all relative group" style={{ height: `${(d.value / max) * 100}%` }}>
                        <span className={`absolute -top-6 left-1/2 -translate-x-1/2 text-xs opacity-0 group-hover:opacity-100 transition-opacity px-1 rounded ${isDark ? 'text-white bg-black' : 'text-black bg-gray-200'}`}>{d.value}</span>
                    </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <div className={`w-full max-w-3xl rounded-3xl border shadow-2xl p-6 relative max-h-[90vh] overflow-y-auto z-10 ${theme.modalBg}`}>
                ...
                <form onSubmit={guardarEdicion} className="space-y-4">
                    ...
                    {/* Simplificado por espacio, el resto sigue la misma lógica de theme.input */}
                    <div className="grid grid-cols-2 gap-4">
                        <div><label className={`text-xs ml-1 ${theme.textSec}`}>Tipo</label><select className={`w-full border rounded-xl p-3 outline-none ${theme.input}`} value={editForm.tipo_registro} onChange={e=>setEditForm({...editForm, tipo_registro:e.target.value})}><option>Diplomado</option><option>Especialización</option></select></div>
